...
public class MyOkHttp {
     ...
     * @param responseHandler 回调
     */
    public void post (Context context, final String url, final Map<String, String> params,
        ...
        if (context == null) {
            request = new Request.Builder().url(url).post(builder.build())
                    .addHeader("Referer", "http://chat.v5kf.com/")
                    .addHeader("Host", "chat.v5kf.com")
     ...
     */
    public void post (String url, String json, IResponseHandler responseHandler) {
        post(null,true, url, json, responseHandler);
    }

    public void post(boolean isRunMain, String url, String json, IResponseHandler responseHandler) {
        post (null, isRunMain, url, json, responseHandler);
    }
     ...
     * @param responseHandler
     */
    public void post (Context context, boolean isRunMain, final String url, final String json, final IResponseHandler responseHandler) {
        ...
        if (context == null) {
            ...
            request = new Request.Builder().url(url).post(requestBody).tag(context).build();
        }


        client.newCall(request).enqueue(new MyCallback(isRunMain, new Handler(), responseHandler));
    }


    public void get (boolean isRunMain, String url, IResponseHandler responseHandler) {
        get(null, isRunMain, url, null, responseHandler);
    }
     ...
     * @param responseHandler 回调
     */
    public void get (boolean isRunMain, final String url, final Map<String, String> params, final IResponseHandler responseHandler) {
        get(null, isRunMain, url, params, responseHandler);
    }
     ...
     * @param responseHandler 回调
     */
    public void get (Context context, boolean isRunMian, final String url, final Map<String, String> params,
            final IResponseHandler responseHandler) {
        //拼接url
        String get_url = url;
        if (params != null && params.size() > 0) {
        ...
        client.newCall(request).enqueue(new MyCallback(isRunMian, new Handler(), responseHandler));
    }
     ...
     * @param responseHandler 回调
     */
    public void upload (Context context, String url, Map<String, String> params, Map<String, File> files,
            final IResponseHandler responseHandler) {
        ...
        if (context == null) {
            request = new Request.Builder().url(url).post(
                    new ProgressRequestBody(multipartBuilder.build(), responseHandler)).build();
        } else {
            request = new Request.Builder().url(url).post(
                    new ProgressRequestBody(multipartBuilder.build(), responseHandler)).tag(context).build();
        }
     ...
     */
    public void download (String url, String filedir, String filename,
     ...
     * @param downloadResponseHandler 下载回调
     */
    public void download (Context context, String url, String filedir, String filename,
            final DownloadResponseHandler downloadResponseHandler) {
        ...
        client.newBuilder().addNetworkInterceptor(new Interceptor() {      //设置拦截器
            ...
            public Response intercept (Chain chain) throws IOException {
                ...
                return originalResponse.newBuilder().body(
                        new ResponseProgressBody(originalResponse.body(), downloadResponseHandler)).build();
            }
        }).build().newCall(request).enqueue(
                new MyDownloadCallback(new Handler(), downloadResponseHandler, filedir, filename));
    }
    ...
    private class MyDownloadCallback implements Callback {
        ...
        public MyDownloadCallback (Handler handler, DownloadResponseHandler downloadResponseHandler, String filedir,
                String filename) {
            mHandler = handler;
    ...
    private class MyCallback implements Callback {
        ...
        private boolean mIsRunMain; //结果是否返回在主线程

        public MyCallback (Handler handler, IResponseHandler responseHandler) {
            this(true, handler, responseHandler);
        }

        public MyCallback (boolean isRunMain, Handler handler, IResponseHandler responseHandler) {
            mHandler = handler;
            mResponseHandler = responseHandler;
            mIsRunMain = isRunMain;
        }
                    ...
                    try {
                        ...
                        if (mIsRunMain) {
                            mHandler.post(new Runnable() {
                                @Override
                                public void run () {
                                    mResponseHandler.onFailure(response.code(),
                                            "fail parse jsonobject, body=" + response_body);
                            ...
                            public void run () {
                                try {
                                    ...
                                    ((GsonResponseHandler) mResponseHandler).onSuccess(response.code(),
                                            gson.fromJson(response_body,
                                                    ((GsonResponseHandler) mResponseHandler).getType()));
                                } catch (Exception e) {
                                    LogUtils.e("onResponse fail parse gson, body=" + response_body, e);
                                    Log.i("myOkhttp", "==gson erro:" + e.toString());
                                    e.printStackTrace();
                                    mResponseHandler.onFailure(response.code(),
                                            "fail parse gson, body=" + response_body);
                                }
